...
<!-- Run: bash scripts/run-prompt.sh program P0 -->
...
Acceptance: `make up` is healthy; `make lint typecheck test` green; `LIVE_API_TESTS=1` smoke tests pass against both vendors; adapters emit cost spans visible in the local Langfuse. Include in the report: the adapter interfaces and the compose services list.

## Execution notes
- Status: DONE on main (scaffold, adapters, compose, CI). Do not re-run; use `program/P1-golden-audio` next.
