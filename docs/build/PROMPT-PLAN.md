...
| program | P12-runtime | partial | — | Runnable product: app images, compose services, Celery worker + beat, /metrics | S,C | images, compose services, per-app queues, workers and beats, /metrics on apps and workers, 10 alert rules, the uc3 dashboard, per-app spend caps and the BudgetExceeded boundary all landed and unit-tested. `make up` starting the product is UNMEASURED: no Docker daemon in a cloud session, so `docker compose config` is proven here and the three image builds in CI. See docs/build/BLOCKERS.md |
