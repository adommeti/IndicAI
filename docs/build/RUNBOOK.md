...
- **One exception, in PR bodies only.** The session proxy appends a single
  `_Generated by [Claude Code](https://claude.ai/code…)_` line to every PR body written from a
  Claude Code session, server-side, on both `POST` and `PATCH` — no client-side setting or check
  can stop it. CI therefore tolerates that exact line when it stands alone, and rejects everything
  else, including the same text inline in a sentence. Commit messages get no such exception:
  `attribution-check.sh`, the `commit-msg` hook and `pre-push` keep history strictly clean.
- **The PR must be opened under your identity — that, not the merge, decides the commit author.**
  GitHub authors a squash commit as the *pull request's* author, whoever presses Merge. In a cloud
  session `gh api` writes go out under the Claude GitHub App, so a PR opened by `scripts/ship.sh`
  is authored `claude[bot]` and lands on `main` the same way, which `attribution-check.sh` rejects.

  | PR | opened via | PR author | merged by | commit on `main` |
  |---|---|---|---|---|
  | #3 | GitHub MCP server | `adommeti` | `adommeti` | `adommeti` — clean |
  | #4 | `gh api` in a session | `claude[bot]` | the session | `claude[bot]` — red |
  | #5 | `gh api` in a session | `claude[bot]` | `adommeti`, in the UI | `claude[bot]` — red |

  PR #5 is the decisive one: a human merged it and it still landed as the bot, so the merger's
  identity is irrelevant. `ship.sh` therefore pushes and then stops when `CLAUDE_CODE_REMOTE` is
  set and no PR exists yet, printing the title and `.claude/run/pr-body.md` to open it with the MCP
  server's `create_pull_request`. Re-running ship finds that PR by head branch and carries on
  through CI and the merge — the merge step itself needs no human. `INDICAI_ALLOW_BOT_PR=1` skips
  the stop, and then the bot address must go into `ALLOWED_AUTHOR_EMAILS` or `main` goes red.
...
| `attribution` job fails with `PR body contains a tool-attribution line` | The session proxy appends `_Generated by [Claude Code](…)_` to every PR body written from a session — on REST `POST` and `PATCH` alike, server-side, after any client check, so neither `.claude/settings.json`'s `attribution.pr=""` nor ship's own grep can prevent it. CI tolerates exactly that one line on its own (see §4), so this failure means something *else* is in the body: a `Claude-Session:` trailer, an emoji, a `claude.ai/share` link, or the footer inline in a sentence. Fix the body, then push — the check reads the event payload, so a corrected body is only seen on the next `synchronize` event, never on a re-run. Never push an empty commit to force one. |
| `ship: … not opening the PR from a cloud session` | Working as intended (§4): a PR opened here would be authored `claude[bot]`, and so would the commit it lands on `main`. The branch is already pushed; open the PR with the MCP server's `create_pull_request` using the printed title and `.claude/run/pr-body.md`, then re-run ship. |
| `ship: could not delete origin/<branch>` | The proxy permits no write to the git-refs path, so a cloud session cannot delete a merged branch, by API or by `git push --delete`. Cosmetic; delete it in the GitHub UI. |
| `attribution-check` FAIL on `main`: author is `claude[bot]` | The PR was *opened* from a cloud session, so GitHub authored its squash commit as the bot — merging it by hand does not help (§4). The bad commit cannot be rewritten once pushed; the failure is confined to that one push event, since the next push to `main` checks a range that excludes it. Open future PRs through the MCP server. |
