...
- **Squash merges are done by a human, not by a cloud session.** A merge is the one API write that
  carries an identity, and in a cloud session it goes out under the Claude GitHub App's own
  credentials — whatever `gh api user` reports — so GitHub authors the squash commit `claude[bot]`.
  `attribution-check.sh` rejects that and `main` goes red. Observed on PR #3 (merged in the UI →
  `adommeti <asdommeti@gmail.com>`, clean) versus PR #4 (merged by a session → `claude[bot]`, red).
  `scripts/ship.sh` therefore stops at a green, mergeable PR when `CLAUDE_CODE_REMOTE` is set;
  merge from the GitHub UI or a local checkout. `INDICAI_ALLOW_BOT_MERGE=1` overrides it, and then
  the bot address must go into `ALLOWED_AUTHOR_EMAILS` or every merge reddens `main`.
...
| `ship: refusing to merge from a cloud session` | Working as intended (§4): the merge would be authored `claude[bot]`. Merge in the GitHub UI or from a local checkout. Everything up to that point — gate, push, PR, CI wait — has already run. |
...
| `attribution-check` FAIL on `main`: author is `claude[bot]` | A merge was performed from a cloud session. The bad commit cannot be rewritten once pushed; the failure is confined to that one push event, since the next push to `main` checks a range that excludes it. Merge from the UI in future (§4). |
