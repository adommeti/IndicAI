...
| `ship: gh CLI is required` | The environment has no `gh` CLI, so `scripts/ship.sh` cannot open the PR, wait for CI or land it. Installing `gh` by hand does not fix it: the session proxy serves REST (`gh api repos/{owner}/{repo}/pulls/{n}`) but returns HTTP 403 for GitHub GraphQL, which `gh pr view`, `gh pr checks` and `gh pr edit` all use. Push with `git push -u origin <branch>` and open the PR through the GitHub MCP server; landing stays with the owner until `gh` ships in the image and ship's PR steps are REST-only. Recorded in `docs/build/BLOCKERS.md`. |
| `attribution` job fails with `PR body contains a tool-attribution line`, but the body you wrote was clean | The PR was opened through the GitHub MCP server, which appends a `Generated by Claude Code` footer. `.claude/settings.json`'s `attribution.pr=""` suppresses that only on the CLI's own PR path. Editing the body removes the footer (the MCP update call does not re-append it), but a workflow re-run replays the original event payload, so the job still sees the old body — the corrected body is only picked up by the next `synchronize` event, i.e. the next real push. Never push an empty commit to force one. |
