...
| T3 | Voice/identity spoofing | Identity comes only from an SSO-populated ASGI principal: `apps/helpdesk_agent/api.py:26-48` (`authenticated_employee`) — no body field, no header, fails closed on a bare deployment. **uc1/P6 added a second identity path and the app's densest egress route, and both belong in this row.** (a) A local development bypass, `AUTH__DEV_BYPASS` in `apps/helpdesk_agent/roles.py`, minting a fixed obviously-fake identity (`dev-bypass@example.test`): refused unless `ENV` is one of dev/local/test/ci, with an unset or misspelt `ENV` refusing too, raised at import AND per request. (b) `GET /sessions/{id}/replay`, gated on uc1's `governance` role — which **means the opposite of uc3's** (ADR 0015): a single Entra group mapped to both apps would grant helpdesk transcript history to exactly the people uc3 segregates from call content. The residual risk lives in the directory, not this repo, and belongs on the `program/P10` deployment checklist. Session ownership re-checked per turn: `persistence.py` raises `PermissionError("Session employee mismatch")`. The action space is closed by type: `action: Literal["answer","clarify","file_ticket"]` (`graph.py:50`), so no identity-affecting action exists to be reached. | `test_api_rejects_unauthenticated_identity` (passed). Employee-mismatch path is covered by `test_postgres_and_langfuse_turns`, which is `slow` and **did not run** here. | **MET** for the identity source and the closed action set; the mismatch check itself is PARTIAL (see note) |
