...
2. **A control on a path nothing executes is not a control.** This matters more in uc3 than it did
   ...
   `api.py:432`, for dispositions. So the hash chain protects two tables that only tests populate,
...
| `pytest platform/tests/test_uc3_spend_and_idempotency.py -q` | **16 passed, 1 failed** — `test_a_whitespace_only_key_is_treated_as_absent`. A real, unfixed defect; see T7 and gap 6. This file was being written by another agent while this review ran, so the count may move. |
| `pytest platform/tests/test_uc3_audit.py -q` | **17 passed, 7 failed** — every failure is a *precondition* assertion poisoned by leftover state in the shared database, not a hash-chain defect. See T7. |
...
| T7 | **Tampering with the audit trail** | `0007_uc3_audit_chain.py` (append-only tables with `prev_hash`/`row_hash`), `0008_uc3_audit_role.py` (`uc3_app` gets `SELECT, INSERT` on the three chained tables and nothing else; `UPDATE, DELETE` revoked from `PUBLIC`), `0009_uc3_chain_anchor.py` (out-of-chain head + row count), `0010_uc3_seq_generated_always.py`. `audit.append` hashes `sha256(prev_hash || canonical_json(row_without_hashes))` under a per-table advisory lock (`audit.py:229-262`); `NOT_HASHED` is validated against the model (`audit.py:101,161-181`). Nightly `uc3.chain_verify` at 05:00 walks all three chains, logs loudly and re-anchors only on a clean walk (`audit.py:614-658`). Neither the alert nor `GET /audit/chain_status` publishes `head_hash` or `first_break_id` (`audit.py:552-580`, `api.py:589-593`). Dispositions are the one production writer and go through `audit.append` with an `Idempotency-Key` unique on `(flag_id, key)` (`api.py:378-461`, `0013_uc3_disposition_idempotency.py`), the key deliberately in `NOT_HASHED`. | `platform/tests/test_uc3_audit.py`: **17 passed, 7 failed**. The 7 are all the `integration` ones and all fail on the same precondition — see the T7 note. The privilege denials themselves *were* observed in this run (the `pytest.raises(ProgrammingError)` blocks inside `test_the_app_role_cannot_update_or_delete_the_audit_tables` passed; its trailing `verify_chain(...).ok` assertion is what failed). Idempotency: `test_a_replayed_key_returns_the_original_receipt_and_appends_nothing`, `test_a_racing_duplicate_resolves_to_the_winners_receipt` (passed). | **PARTIAL** — four limits, below |
...
4. **A whitespace `Idempotency-Key` breaks the append-only guarantee it was added to protect.**
   `api.py:418` is `key = idempotency_key.strip() if idempotency_key else None`, which yields `""`
   — not `None` — for a header of spaces. `""` is not distinct under
   `uq_dispositions_flag_idempotency_key`, so a *second, different* ruling on the same flag from a
   client whose proxy sends a blank header hits the constraint, takes the `if not key: raise` arm
   at `api.py:452` and returns 500 with the reviewer's changed mind lost on a table that cannot be
   repaired. Caught by `test_a_whitespace_only_key_is_treated_as_absent`, which **fails today**.
   *Smallest fix:* `key = (idempotency_key or "").strip() or None`.
...
| 4 | Residency decision recorded in ADR; DPA/vendor risk review status noted (T4) | **✘ NOT MET** | There is **no ADR** recording uc3's cross-border position. uc1 has ADR 0014; uc3 has nothing, and uc3 is the app whose data B5 classifies Confidential/regulated and whose Claude leg is unredacted. DPA and vendor risk review: **not passed**, same as uc1. Half of this line is a document that does not exist and the other half is a gate nobody has been through. *Smallest fix:* an ADR modelled on 0014, narrower — synthetic-only, unredacted text, no audio, conditional on Gate 0 and a DPA. |
...
| 8 | No secrets in repo; secret scanner in pre-commit; Langfuse header redaction (T8) | **✔ MET** | `.gitignore:1-2`; only `.env.example` tracked; `detect-secrets` in pre-commit and a wider all-tracked-files scan in the gate and CI — run here, clean. No header, URL or error body ever reaches a span (`runtime.py:117-129`, `langfuse.py:48`), and uc3's presigned URL is never logged or traced. Note for completeness, not as a finding: `platform/config/settings.py:6-7` loads `.env`/`.env.stack` with `python-dotenv` at import, which is how configuration reaches the process; no secret is read by this review and none appears in this file. |
...
**Of the 12 F4 lines: 5 met** (2, 3, 8, 10, 12), **6 partial** (1, 5, 6, 7, 9, 11), **1 not met**
(4), **0 N/A**. Unlike uc1, no line is N/A: T7 is scoped to this app, so the line uc1 could decline
is the one uc3 owns. Of the six partials, line 7 is the one to read first, and not for the reason
its wording suggests: the chain is well built, and the tables it protects are empty.
...
**Residency decision: NOT RECORDED.** PRD B5 offers uc3 option (b) — "minimize what reaches Claude
...
disabled. Nothing in this build authorises processing a real recorded call. Residency is also open
...
| 10 | **T4 residency ADR** | Could have been written and was not. Not blocked by anything external: uc1 wrote ADR 0014 in the equivalent prompt. Listed here as a debt, not an impossibility. |
...
| 6 | A whitespace-only `Idempotency-Key` persists `""`, so a second disposition on that flag 500s and is lost | T7 | `key = (idempotency_key or "").strip() or None` at `api.py:418`; `test_a_whitespace_only_key_is_treated_as_absent` already exists and fails | uc3 owner |
