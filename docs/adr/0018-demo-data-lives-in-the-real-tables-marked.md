...
- `analysis_runs.model` is `demo-seed` (`demo_seed.py:110`), never a vendor model id;
- `analysis_runs.prompt_version` is empty (`demo_seed.py:116`), because no prompt ran;
- `analysis_runs.output` carries `{"demo": true, "source": "golden:<id>"}`;
- `calls.source_key` is prefixed `demo/uc3/` (`demo_seed.py:107`), which is also the
  uniqueness that makes re-seeding a no-op.

`metrics.DEMO_MARKER` (`metrics.py:316`) and `metrics._real_runs_only`
(`metrics.py:319`) drop every flag hanging off a demo run from `flag_statement` and
`qa_sample_statement`, which is every read path behind `/metrics/precision`,
`/metrics/false_negative_estimate` and `/qa-sample`. Both metrics responses return a
`demo_excluded` count (`api.py:582`, `api.py:605`) rather than filtering silently: an
operator has to be able to tell an empty dashboard from one whose every row was removed.

`demo_seed.seed` refuses any `ENV` outside the dev/local/test/ci allow-list, and checks
that itself (`demo_seed.py:639`) rather than in the CLI, because the module ships inside
...
verdicts as evidence: every disposition carries `DEMO_NOTE` (`demo_seed.py:127`) saying
...
with the function and model that made it. A gloss and a finding are different claims:
the translation is genuine, and the row carrying it is still `demo-seed`.
...
- Any future read path that counts flags or dispositions must apply `DEMO_MARKER`. This
  is a standing obligation on `metrics.py` and on anything that grows a new aggregate.
...
- The chain still verifies, anchors included:
  `platform/tests/test_uc3_demo_seed.py:389` (`integration`)
- The stored columns say `demo-seed`, and no cost is claimed:
  `platform/tests/test_uc3_demo_seed.py:484` (`integration`)
- Seeding does not move precision or the false-negative estimate:
  `platform/tests/test_uc3_demo_seed.py:524` (`integration`)
- `seed` refuses production itself: `platform/tests/test_uc3_demo_seed.py:156`
